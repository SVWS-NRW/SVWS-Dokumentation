...
Der SVWS-Server trennt die Daten der Schulen im Gegensatz zu SchILDzentral auf Datenbankebene. Dies führt dazu, dass pro Schule ein Schema verwendet wird. Hier kann, anders als in SchILDzentral, über den Datenbank-Benutzer sichergestellt werden, dass die Daten einer Schule auch nur den Benutzern dieser Schule zur Verfügung stehen.

Durch diese Trennung wird auch das Einspielen von Backups wesentlich vereinfacht, wenn Daten von nur einer Schule wieder eingespielt werden müssen.

Durch das neue Datenbank-Schema-Management in SchILD-NRW und auch in SchILDzentral können diese Schemata verwaltet werden. Manuelle Eingriffe in die einzelnen Datenbanken entfallen zukünftig, da die Aktualisierung von Statistikdaten und die Anhebung der Datenbankrevisionen automatisch vom SVWS-Server übernommen werden.
...
Die Trennung der Mandantendaten auf Tabellenebene durch Mandanten-Schlüssel (z.B. ein „Mandant_ID“-Feld in jeder Zeile) bietet zwar eine logische Abgrenzung, stellt aber bei hohem Schutzbedarf kein ausreichendes Sicherheitsniveau dar. Dies hat mehrere Gründe:
...
Bei hohem Schutzbedarf – insbesondere bei sensiblen, personenbezogenen oder geheimhaltungsbedürftigen Daten – fordern viele gesetzliche und branchenspezifische Vorgaben (z.B. BSI IT-Grundschutz, ISO 27001, DSGVO) eine möglichst weitgehende technische Trennung, was in der Praxis meist durch separate Schemata umgesetzt wird.
5. **Vermeidung von Datenvermischung:**
Bei Migrationen, Backups oder Wiederherstellungen besteht bei einem gemeinsamen Schema die Gefahr, dass versehentlich Daten anderer Mandanten mit exportiert, importiert oder wiederhergestellt werden. Separate Schemata verhindern dies zuverlässig.

**Fazit:**
Die Trennung der Mandantendaten in unterschiedliche Schemata bietet ein deutlich höheres Sicherheitsniveau als die Trennung per Mandanten-Schlüssel in den Tabellen. Sie reduziert die Angriffsfläche, minimiert das Risiko von Datenlecks durch menschliche oder technische Fehler und entspricht den Anforderungen bei hohem Schutzbedarf. Bei besonders sensiblen Daten ist diese Form der Trennung daher zwingend erforderlich.


## Datenbanksystem MariaDB

Der SVWS-Server unterstützt MariaDB als Open-Source-Datenbanklösung. Hier kann auf die kostenfreie Community-Variante, aber auch auf die Enterprise-Lösung zurückgegriffen werden.
...
Die Pflege von fehleranfälligen Doppelungen in den Abfragen, um mehrere Datenbanksysteme zu unterstützen, entfällt durch die Auswahl eines einzigen DBMS.
...
Die Anzahl der individuellen Einzellösungen kann hier reduziert werden, auch durch die Bereitstellung von versionierten Standard-API-Endpunkten, wie z.B. die Schnittstelle zur Synchronisation von Lernplattformen, die hier eine deutliche Verringerung von Aufwänden durch Automatisierung hervorbringen kann.
