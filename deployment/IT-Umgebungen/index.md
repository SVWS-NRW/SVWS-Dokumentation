...
Kleine Grundschulen haben die Schulverwaltungssoftware gegebenenfalls auf einem Rechner installiert, größere Schulen besitzen unter Umständen einen oder mehrere Server. Andere Schulen haben IT-Dienstleister, die sie mit Serverinstallationen unterstützen. Am oberen Ende findet sich der zentrale Einsatz in Rechenzentren.

## Einzelplatz-Rechner

Mit Hilfe des Windows-Installers können der SVWS-Server und SchILD-NRW 3 auf einem einzelnen PC mit Windows 11 Betriebssystem installiert werden.
...
Auch hier wird der Port 443 intern geöffnet, so dass der SVWS-Client mit einem Web-Browser im Verwaltungsnetz aufgerufen werden kann.

Der SVWS-Server kann hierbei sowohl auf einem Windows- als auch auf einem Linux-basierten Rechner betrieben werden.

Die Installation auf Linux-Basis ermöglicht den Schulen beziehungsweise den Schulrägern eine komplett lizenzkostenfreie Installation des SVWS-Servers.

Das bei der Installation erstellte Zertifikat des Servers sollte an die Clients per Gruppenrichtlinie oder manuell verteilt werden, damit der Browser die Verbindung als *sicher* einstuft und keine Warnung beim Aufrufen ausgibt.
...
Perspektivisch wird der Zugriff auf die MariaDB durch Nutzung der entsprechenden REST-Schnittstelle des SVWS-Servers ersetzt werden, so dass nur noch eine Verbindung zum SVWS-Server benötigt wird.
...
In größeren Umgebungen sind verschiedene Varianten der Installation denkbar. So können mehrere SVWS-Server auch die Funktionalität von SchILD-NRW*zentral* übernehmen.
...
In Rechenzentren macht es aus Lizenzgründen Sinn, auf Linux-Systeme zu setzen, so dass hier mit Containern gearbeitet werden kann. Des Weiteren können auch einzelne virtuelle Maschinen (KVM) in Virtualisierungssystemen erstellt und den jeweiligen Schulen zugeordnet werden. Es ist auch möglich, jedoch eher für Schulsysteme oder Dependenzen sinnvoll, mehrere Schul-Schemata über einen SVWS-Server bereitzustellen.
