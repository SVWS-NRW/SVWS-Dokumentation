...
class File2Constraints : public IntMinimizeScript {
protected:
...
public:
	File2Constraints(const Options& opt)
	{
		...
		while (std::getline(infile, line))
		{
			...
			{
				if (hasObjective) {
					std::cout << "Duplicate objective declaration!";
					throw "Duplicate objective declaration!";
				}
				...
				IntArgs intArgs(coefficients);
				BoolVarArgs varArgs(variables);
				objective = IntVar(*this, min - 1, max + 1);
				linear(*this, intArgs, varArgs, IRT_EQ, objective);
		...
		if (!hasObjective) {
			throw "No objective function defined!";
		}
	...
	virtual void
		print(std::ostream& os) const {
			...
			if (true) {
				os << "Current objective " << objective << std::endl;
	...
	File2Constraints(bool share, File2Constraints& s) : IntMinimizeScript(share, s) {
...
main(int argc, char* argv[]) {
	...
	Script::run<File2Constraints, BAB, Options>(opt);
	return 0;
}
