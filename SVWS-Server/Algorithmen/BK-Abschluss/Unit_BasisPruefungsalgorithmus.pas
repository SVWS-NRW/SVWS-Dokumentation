...
begin
  ...
  with fQryFachGr do
	begin
		CommandText := 'SELECT * FROM ZusatzInfos';
		FSGL_BKAbschl := '';
		FJG_BKAbschl := '';
		Open;
		while not EOF do
...
var
	prf10, cmd: string;
	warnung, zp10fach, hinzu, ist_fs: boolean;
...
//										 ( fQryS.FieldByname( 'FachSchriftlichBA' ).AsString = '+' ) and ( ( FSchulGliederung = 'C01' ) or ( FSchulGliederung = 'A02' ) ),
										 ( fQryS.FieldByname( 'FachSchriftlichBA' ).AsString = '+' ) and ( ( FSchulGliederung = 'C01' ) ),
										 nabschl,
                     nabschl_ba,
...
//					Meldung( Format( '%s: Bitte Abschlussnote manuell eingeben', [ FieldByname( 'FachName' ).AsString ] ) );
  ...
  begin
    ...
    try
      with qryZK do
      begin
        CommandText := Format( 'select Fach_ID, NoteAbschluss from SchuelerBKFaecher where Schueler_ID=%d', [ s_id ] );
        Open;
        while not Eof do
        begin
