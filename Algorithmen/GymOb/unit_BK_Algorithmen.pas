  ...
  TBestehungswert = (bwBestanden, bwNichtBestanden, bwIgnorieren );
	...
	TBKPruefungsAlgorithmus = class( TPruefungsAlgorithmus )
      ...
      function Pruefe_APO_BK_15_Abschluss_E: string;
			...
			function Pruefe_APO_BK_15_Abschluss_D01: string;
...
// Bildungsgang D01
    end else if FBK_Aktiv and IstInListe( fPruefOrdnung, C_PO_GLIEDERUNG_D_BAB ) and ( FBKGliederung = glD01 ) then
    begin
      Result := Pruefe_APO_BK_15_Abschluss_D01;
      Fabbruch := true;
//---------------------------------------------------------------------------------------------
...
// Prüfen, ob in einem Abschnitt das "Zertifikat berufl. Kenntnisse erreicht (nur bei C03-Gliederungen)
	...
	begin
{$IFDEF UNIDAC}
		qry := CreateForwardDataset( fConSchild );
{$ELSE}
...
//			qry.CommandText := Format( 'SELECT ID FROM SchuelerLernabschnittsdaten WHERE Schueler_ID=%d AND Abschluss_B=%s', [ s_id, QuotedStr( 'APO-BK-03/C-BFS-BK/BK' ) ] );
      cmd :=  Format( 'SELECT ID FROM SchuelerLernabschnittsdaten WHERE Schueler_ID=%d AND Abschluss_B=%s',
...
{$IFDEF UNIDAC}
	qry := CreateForwardDataset( fConSchild );
	try
		qry.SQL.Text := 'SELECT Kennung FROM EigeneSchule_Fachklassen WHERE ID=' + IntToStr( FFachklasse_ID );
		qry.Open;
...
/////// D01
function TBKPruefungsAlgorithmus.Pruefe_APO_BK_15_Abschluss_D01: string;
var
	idf, msg: string;

	function Zugelassen_D01: boolean;
	var
		i : integer;
		res: string;
	begin
...
//  2. FS raus
    ...
    begin
      ...
      while not Eof do
      begin
        if ( FieldByname( 'Reihenfolge' ).AsString <> '' ) and ( FieldByname( 'Reihenfolge' ).AsString <> '1' ) then
          Delete
        else
          Next;
      end;
      First;
		...
		ResetBKAbschluss( fS_ID );
		TransactionHandler.DoExecute( Format( 'UPDATE SchuelerBKFaecher SET MdlPruefungFW=%s, NoteAbschluss=NULL WHERE Schueler_ID=%d',
															[ QuotedStr( '-' ), fS_ID ] ) );
...
// Auf 6 in C0 prüfen
	...
	function AbschlussBerechnen_D01: boolean;
	var
		idf, idfa: string;
		i : integer;
    inote: integer;
    ...
    nprfg: string;
	begin
    if FeldWertAnzahl( fC0, 'IstSchriftlichBA', '+' ) < 2 then
    begin
      Meldung( 'Falsche Anzahl schriftlicher Fächer' );
      exit;
    end;
    ...
    if FeldWertAnzahl( fC0, 'NotePrfGesamt', '999' ) > 0 then
    begin
      Meldung( 'Abschlussnoten unvollständig' );
      Result := false;
      exit;
    ...
    if FNoteFachpraxis = '' then
    begin
      Meldung( 'Note Fachpraxis fehlt' );
      Result := false;
      exit;
    end;

    if FNotePraktischePruefung = '' then
    begin
      Meldung( 'Note praktische Prüfung  fehlt' );
      Result := false;
      exit;
    end;
    ...
    if Result then
    begin
      Result := FNotePraktischePruefung <= '4';
      if not Result then
        Meldung( 'Note praktische Prüfung nicht ausreichend' );
    end;

    if Result then
  		Meldung( 'Abschluss erreicht' );
	end;
...
begin
	...
	'?' : Meldung( 'Die Prüfungsordnung des Schülers/der Schülerin umfasst eine Abschlussprüfung. Bitte verwenden Sie die Kartei-Seite "BK-Abschluss".' );
	'Z' : // Zulassung prüfen
				if Zugelassen_D01 then
					Result := fBK_Modus
				else
					Result := 'N';
	'A' : // Abschluss
				if AbschlussBerechnen_D01 then
					Result := 'A';
	end;
end;
