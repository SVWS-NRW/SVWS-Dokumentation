...
Für **alle anderen API-Zugriffe**, die keine direkten Datenbankadministrationsaufgaben ausführen, erfolgt die Anmeldung mit einem **SVWS internen Benutzerkonto**, das innerhalb des jeweiligen schulbezogenen Datenbestands über die erforderlichen Berechtigungen verfügt. Dies kann beispielsweise ein Benutzerkonto der Schulleitung mit einem entsprechenden SVWS-Account sein.
...
Eine maschinenlesbare API-Beschreibung ermöglicht es Programmen, Entwicklungswerkzeugen und insbesondere KI-gestützten Anwendungen, die vorhandenen Schnittstellen automatisiert zu analysieren und gezielt zu nutzen.

## Stand Release Kandidat 1.4

Die Anzahl der definierten HTTP-Endpunkte nach Methode ist:

| HTTP-Methode | Anzahl Endpunkte |
| -- | -- |
| GET | 302 |
| POST | 223 |
| PATCH | 157 |
...
Insgesamt: **853 Endpunkte**.

## API per Curl aufrufen

In der oben beschriebenen Swagger-Oberfläche kann sowohl in der Beschreibung eines Endpunkts als auch nach dessen erfolgreicher Ausführung der zugehörige curl-Befehl angezeigt werden.

Die folgende Abbildung zeigt ein Beispiel:

![Swagger Curl Aufruf](./graphics/Swagger_curl.png)

Es handelt sich hierbei um einen GET-Aufruf.
Dabei wird aus der Datenbank *meineSchule* im Abschnitt mit der *ID 1* die Lernplattform mit der *ID 1* ausgelesen. Mit dem Parameter *-u* kann der berechtigte User samt Passwort übergeben werden.

Hier der vollständige Curl Befehl:
